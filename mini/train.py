...
def train(env: MicroRTSGridModeVecEnv, params: Parameters, agent: AbstractAgent, optimizer: optim.Optimizer):
    ...
    for epoch in range(params.epochs):
        ...
        observations_tensor = torch.tensor(observations, device=params.device, dtype=torch.float32)
        
        for step in range(params.max_steps):
            invalid_action_masks = env.get_action_mask()
            invalid_action_masks_tensor = torch.tensor(invalid_action_masks, device=params.device)

            actions, log_probs = agent.forward(observations_tensor, invalid_action_masks_tensor)
            ...
            trajectory['rewards'].append(torch.tensor(rewards, device=params.device))
            trajectory['dones'].append(torch.tensor(dones, device=params.device))

            if dones.all():
                break
        
        # Compute returns
        discounted_rewards = torch.zeros((params.batch_size, step + 1), device=params.device)
        R = torch.zeros(params.batch_size, device=params.device)
        for t in reversed(range(step + 1)):
            R = trajectory['rewards'][t] + params.gamma * R * (~trajectory['dones'][t])  # Apply gamma and mask done games
        ...
        # Compute loss
        log_probs = torch.stack(trajectory['log_probs'], dim=1)  # Shape: [batch_size, step+1]
